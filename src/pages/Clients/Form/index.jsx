...
export default function ClientForm({ onSubmit, forEdit, visible, onCancelSubmit, title, clearFields }){
    ...
    const registerClientSchema = yup.object().shape({
        whatsapp: yup.
        string()[ !forEdit ? 'required' : 'notRequired']('Informe o número do Whatsapp').
        min(11,'Número de Whatsapp inválido'),
        contactNumber: yup.string().notRequired('Informe o número para contato').
        min(11,'Número para contato inválido'),
        fullName: yup.string()[ !forEdit ? 'required' : 'notRequired']('Informe o nome completo').
        max(150,'O nome deve possuir no máximo 150 caracteres'),
        adress: yup.string().notRequired().
        max(100,'O endereço deve possuir no máximo 150 caracteres'),
        neighborhood: yup.string().notRequired().
        max(65,'O bairro deve possuir no máximo 65 caracteres'),
        houseNumber: yup.string().notRequired().
        max(100000,'O Número da casa deve possuir no máximo 100000 caracteres'),
    ...
    const registerClientValidation = useFormik({
        ...
        onSubmit: async values => await onSubmit({
            ...values,
            contactNumber: convertToPhoneId(values.contactNumber).normal,
            whatsapp: values.whatsapp.length > 0 ? convertToPhoneId(values.whatsapp).withId : ''
        })
    });
